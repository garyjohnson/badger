...
function installHooks () {
  ...
  const badgerHooks = {
    Stop: { matcher: '', hooks: [{ type: 'command', command: 'badger hook stop' }] },
    SessionStart: { matcher: '', hooks: [{ type: 'command', command: 'badger hook session-start' }] }
  }

  for (const [event, hookEntry] of Object.entries(badgerHooks)) {
    if (!settings.hooks[event]) settings.hooks[event] = []
    const exists = settings.hooks[event].some(h =>
      (h.hooks || []).some(inner => (inner.command || '').includes('badger hook'))
    )
    if (!exists) {
